...
int main()
{
  ...
  cin>>flag;
  ifstream infile;
  double del,k,k_A,a0,z0,t0,Cx=0;
  if(flag==0){
...
while(num<N){
  ...
  while(1){
    ...
    dfdx[0][0]=-h/2.0*(d2Uz+(Cx+Ax+Ab)*d2Ab/del+dAb*dAb/del);
    dfdx[0][1]=1.0+h/2.0*(Cx+Ax+Ab)*dAb/(del*del);
    dfdx[1][0]=1.0-h/2.0/(del*del)*(Cx+Ax+Ab)*dAb;
    dfdx[1][1]=h/2.0/pow(del,3)*(1+(Cx+Ax+Ab)*(Cx+Ax+Ab));
    b1=b[0]-f[0];
    b2=b[1]-f[1];
